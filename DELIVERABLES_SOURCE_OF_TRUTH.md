...
| `src/app/admin/page.tsx` | SRS 5.1 / PRD §H: **System Admin Dashboard** – pricing config, tool activation, revenue monitoring, institutional license management (placeholder), user activity logs (placeholder); **Save Configuration** CTA. |
| `src/app/institution/admin/page.tsx` | SRS 5.1 / UI/UX: **Institutional Admin Panel** – manage seats & monitor usage; **Assign Seat** CTA (placeholder until backend). |
...
| `src/components/AnalysisTypeSelector.tsx` | Six analysis types; descriptions updated for full workflows; removed supportingOnly. Reads `thesisAnalyzer_adminTools` from localStorage and disables types when admin has turned tool OFF (FR-019). | PRD §D; SRS FR-019. |
...
| `src/app/auth/page.tsx` | Login form submit button label set to **Login** (was Sign In). | UI/UX Screen List: Login primary CTA = **Login**. |
| `src/app/error/page.tsx` | CTAs order: **Retry** first, then **Go to Payment**. | UI/UX: Error screen **Retry / Go to Payment**. |
| `src/app/history/page.tsx` | Copy updated: snapshot cards show **View Details**; empty state **New Analysis**. | UI/UX: History list primary CTA **View Details**. |
| `src/app/history/[id]/page.tsx` | Primary CTA **Re-download Report** (disabled placeholder until backend). | UI/UX: History detail primary CTA **Re-download Report**. |
| `src/app/admin/page.tsx` | Added sections: Institutional license management, User activity logs (placeholders). | PRD §H: admin-controlled entails all functionality. |
...
- **Routes aligned with SRS 5.1 and UI/UX**  
  ...
  Authenticated: `/dashboard`, `/analysis`, `/analysis/advanced`, `/history`, `/history/[id]`, `/subscription`, `/payment`, `/institution/admin`.  
  Admin: `/admin`.  
  Error: `/error`.
...
- **Admin**  
  System admin dashboard: pricing configuration, tool activation (saved to localStorage; disabled tools appear disabled in analysis UI per FR-019), revenue monitoring, institutional license management (placeholder), user activity logs (placeholder). **Save Configuration** CTA.
- **Institutional Admin**  
  Route `/institution/admin`: Institutional Admin Panel – manage seats & monitor usage; **Assign Seat** CTA (placeholder until backend).
...
- **Copy and CTAs**  
  Landing CTA = **Start Analysis**; Login screen CTA = **Login**; Register = **Create Account**; Pricing = **Choose Plan** / **Contact Sales**; Payment = **Complete Payment**; Dashboard = **New Analysis**; History list = **View Details** (on snapshot cards) + **New Analysis** (empty state); History detail = **Re-download Report**; Subscription = **Upgrade / Renew**; Admin = **Save Configuration**; Error = **Retry** / **Go to Payment**; Institutional Admin = **Assign Seat**. Header shows **Login**; nav includes Pricing, Dashboard (when auth), History (when auth), Admin (when role ADMIN).
